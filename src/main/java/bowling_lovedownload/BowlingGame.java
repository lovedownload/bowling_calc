...
public class BowlingGame {
	...
	public void TenFrame(int i) {
		...
		for(int j = i; j < 20; j++)
		{
			...
			if(pin[j] < 10)
			{
				SpareScore(j);
				System.out.println();
				SecondPinCheck(pin[j+1], j);
			SecondPinCheck(pin[lastJ + 1], lastJ);
	...
	public void GameOver() {
		try {
			System.out.println();
	public int SecondPinCheck(int second_pin, int i) {
		while(second_pin + pin[i] > 10 || second_pin + pin[i] < 0)
			second_pin = clearpin.nextInt();
		return second_pin;
			System.out.println();
			e.printStackTrace();
		}
	...
	public void SetSecondPin(int second_pin, int i)
		pin[i+1] = second_pin;
